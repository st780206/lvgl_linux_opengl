...
void _lv_disp_refr_timer(lv_timer_t * tmr)
{
    ...
    //LV_LOG_INFO("--------------- start ----2222222222------>> ");
    lv_refr_join_area();

    //LV_LOG_INFO("--------------- start ----55555555555------>> ");
    lv_refr_areas();

    //LV_LOG_INFO("--------------- start ----6666666666------>> ");
    /*If refresh happened ...*/
...
static void lv_refr_join_area(void)
{
    ...
    //LV_LOG_INFO("--------------- AAAAAAAAA ----111111111------>> ");
    for(join_in = 0; join_in < disp_refr->inv_p; join_in++) {
        if(disp_refr->inv_area_joined[join_in] != 0) continue;

        //LV_LOG_INFO("--------------- AAAAAAAAA ----2222222------>> ");
        ...
        for(join_from = 0; join_from < disp_refr->inv_p; join_from++) {
            ...
            //LV_LOG_INFO("--------------- AAAAAAAAA ----55555555------>> ");
        }
        //LV_LOG_INFO("--------------- AAAAAAAAA ----666666------>> ");
    }
    //LV_LOG_INFO("--------------- AAAAAAAAA ----777777------>> ");
}
...
static void lv_refr_areas(void)
{
    ...
    for(i = 0; i < disp_refr->inv_p; i++) {
        ...
        if(disp_refr->inv_area_joined[i] == 0) {
            ...
            disp_refr->driver->draw_buf->last_part = 0;
            //LV_LOG_INFO("--------------- BBBBB ----3333333333------>> ");
            lv_refr_area(&disp_refr->inv_areas[i]);
            //LV_LOG_INFO("--------------- BBBBB ----44444444------>> ");

            px_num += lv_area_get_size(&disp_refr->inv_areas[i]);
        }
    }
    //LV_LOG_INFO("--------------- BBBBB ----555555555------>> ");
}
...
static void lv_refr_area(const lv_area_t * area_p)
{
    //LV_LOG_INFO("--------------- CCCCCC ----1111111111------>> ");
    /*With full refresh just redraw directly into the buffer*/
    //LV_LOG_INFO(" ========   disp_refr->driver====  %p    ========================== ", disp_refr->driver);
    if(disp_refr->driver->full_refresh) {
    ...
    //LV_LOG_INFO("--------------- CCCCCC ----333333333---- max_row=%d  draw_buf->size=%d  w=%d  -->> ", max_row, draw_buf->size, w);
    ...
    //LV_LOG_INFO("--------------- CCCCCC ----44444444444------>> ");

    /*Always use the full row*/
    lv_coord_t row;
    lv_coord_t row_last = 0;
    //LV_LOG_INFO("--------------- CCCCCC ----555555555555-----   row=%d    area_p->y1=%d  max_row=%d ->> ",
    //        row, area_p->y1, max_row);
    for(row = area_p->y1; row + max_row - 1 <= y2; row += max_row) {
        ...
        lv_refr_area_part(area_p);
//        LV_LOG_INFO("--------------- CCCCCC ----666666666-----   row=%d    area_p->y1=%d  max_row=%d ->> ",
//                row, area_p->y1, max_row);
    }

    //LV_LOG_INFO("--------------- CCCCCC ----777777------>> ");
